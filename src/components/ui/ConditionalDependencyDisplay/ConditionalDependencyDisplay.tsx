...
export function ConditionalDependencyDisplay({
  ...
  const renderDateCondition = () => {
    ...
    return (
      <div className="flex items-start gap-2">
        <div className={`${iconSizeClasses[size]} flex-shrink-0 mt-0.5 ${datePassed ? 'text-danger-500' : 'text-warn-500'}`}>
          <svg fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clipRule="evenodd" />
          </svg>
        </div>
        <div className="flex-1">
          <div className={`${sizeClasses[size]} text-[var(--text-secondary)]`}>
            {datePassed ? (
              <span className="text-danger-600 font-medium">Date passed: {formattedDate}</span>
            ) : (
  ...
  const renderUserDependencies = () => {
    ...
    return (
      <div className="flex items-start gap-2">
        <div className={`${iconSizeClasses[size]} flex-shrink-0 mt-0.5 ${allUsersConfirmed ? 'text-success-500' : 'text-warn-500'}`}>
          <svg fill="currentColor" viewBox="0 0 20 20">
            <path d="M9 6a3 3 0 11-6 0 3 3 0 016 0zM17 6a3 3 0 11-6 0 3 3 0 016 0zM12.93 17c.046-.327.07-.66.07-1a6.97 6.97 0 00-1.5-4.33A5 5 0 0119 16v1h-6.07zM6 11a5 5 0 015 5v1H1v-1a5 5 0 015-5z" />
          </svg>
        </div>
        <div className="flex-1">
          <div className={`${sizeClasses[size]} text-[var(--text-secondary)] mb-1.5`}>
            {allUsersConfirmed ? (
    ...
    <div className={`space-y-3 ${className}`} {...props}>
      ...
      {conditionsMet !== undefined && (
        <div className={`flex items-center gap-2 ${sizeClasses[size]}`}>
          {conditionsMet ? (
            ...
            <>
              <div className={`${iconSizeClasses[size]} text-warn-500`}>
                <svg fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clipRule="evenodd" />
                </svg>
              </div>
              <span className="text-warn-600 font-medium">Waiting for conditions to be met</span>
            </>
          )}
        </div>
      )}
      ...
      {conditionalType === 'both' && (
        <div className="flex items-center gap-3">
          ...
          <span className={`${sizeClasses[size]} text-[var(--text-muted)] font-medium uppercase`}>OR</span>
          <div className="flex-1 h-px bg-[var(--border-default)]" />
        </div>
      )}
      ...
      {conditionalType === 'both' && !compact && (
        <div className={`${sizeClasses[size]} text-[var(--text-muted)] italic border-l-2 border-gray-300 pl-3`}>
